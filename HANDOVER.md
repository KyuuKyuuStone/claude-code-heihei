...
- **CI 现有 5 个 job：server-tests / server-tests-linux-exp / adapters / docs / desktop-tests**：v1.2.0 新增 `desktop-tests`（windows runner，18 文件起步），**v1.2.6 扩容至 37 文件**（达「连续 ≥3 次全绿」标准，1104 tests 逐字一致）；**v1.2.6 另加 `server-tests-linux-exp` 实验 job**（ubuntu + `continue-on-error`，一次性采集 Linux 失败清单、**用完即撤**，与 windows 门禁**并列不替换**——收编评估见 `cc-heihei-交接/批次7_Linux收编评估_20260919.md`）
