...
title: 项目结构
nav_title: 项目结构
description: 仓库的目录职责边界：CLI、Server、桌面端与文档站。
order: 3
---

# 项目结构

本项目包含 CLI/TUI、本地 Server、Electron 桌面端和文档站。下面只列稳定的职责边界；具体文件以当前源码为准。

```text
.
├── bin/
│   └── claude-heihei                 # CLI 启动脚本
...
│   ├── sidecars/
│   │   └── claude-sidecar.ts       # server / cli 统一入口
│   ├── scripts/                    # 构建、打包和资源准备
...
| `src/server/index.ts` | Bun / `Bun.serve` | 本地 HTTP、WebSocket |
