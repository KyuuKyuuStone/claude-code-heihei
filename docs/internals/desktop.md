...
title: 桌面端架构
nav_title: 桌面端架构
description: Electron 主进程、Server Sidecar 与 CLI 子进程的进程边界。
order: 1
---
...
- **Electron main** 负责窗口、系统对话框、更新、终端、原生预览和 Sidecar 生命周期。
- **Renderer** 只负责界面，通过 preload 暴露的 `window.desktopHost` 使用原生能力。
- **Server Sidecar** 是桌面端共用的本地服务，提供 REST、WebSocket、Provider 代理和会话管理。
...
5. Server 在用户开始会话时按需启动 CLI 子进程。

Server 可绑定局域网可访问地址，但桌面 Renderer 使用 loopback 控制地址。新增可远程访问的能力时，要单独设计对应的 Token 与能力限制，不能因为本地 Server 已启动就假定所有来源都可信。
...
Sidecar 在导入业务模块前设置 `CLAUDE_APP_ROOT`、`CALLER_DIR` 和启动参数，因为 Server 与 CLI 的顶层模块会读取这些值。
...
- `/api/*` REST 请求
- `/ws/:sessionId` 桌面客户端连接
- `/sdk/:sessionId` CLI 内部连接
- OAuth 回调
- 受限的预览与本地文件访问
- 打包后的静态资源

鉴权规则按客户端能力区分。新增路由时，必须先确定它属于本地桌面、内部 SDK 还是公开静态资源，再放进对应的认证和 CORS 边界。
...
本地模型是一个独立的 `llama-server` 子进程，桌面端拉起它、等待就绪，再把它的 loopback 端口注册成 `anthropic` 格式的 Provider。细节和内存模型见[本地模型原理](./local-model.md)。
...
| Provider、Computer Use 等设置 | Server 管理的配置文件 |
