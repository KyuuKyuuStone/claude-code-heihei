---
title: Claude Code Heihei 是什么
nav_title: 这是什么
description: 跑在你自己电脑上的 AI 编程工作台：模型自己接，改动你审阅。
order: 0
---

# Claude Code Heihei 是什么
...
Claude Code 是 Anthropic 出的命令行编程 Agent，跑在终端里。Claude Code Heihei 的内核就是从 Claude Code 源码修复而来的一份 CLI（仓库里叫 `claude-heihei`），桌面端是包在它外面的图形界面。
...
**本地大模型。** 不想联网、也不想花 API 钱，就让它在你自己电脑上跑一个 GGUF 模型，数据不出本机。
