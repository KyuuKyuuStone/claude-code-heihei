...
The CLI is the core of Claude Code Heihei — every desktop session runs one underneath. If you only want the graphical app, installing that is enough; see [Download and install](../start/install.md). The steps below are for people who want a terminal workflow, `--print` automation, or a source checkout to read and contribute to.
...
```bash
./bin/claude-heihei
./bin/claude-heihei -p "Summarize the directory structure of this project"
```
...
`./bin/claude-heihei` only works inside the checkout. Put it on your `PATH` and you can type `claude-heihei` in any project directory — the CLI treats the current working directory as the project root.

On macOS and Linux, add this to `~/.bashrc` or `~/.zshrc`:

```bash
# Option 1: add to PATH (recommended)
export PATH="$HOME/path/to/claude-code-heihei/bin:$PATH"

# Option 2: alias
alias claude-heihei="$HOME/path/to/claude-code-heihei/bin/claude-heihei"
```
...
If `claude-heihei` runs on Windows or Git Bash while Node, Python, uv, and bun live inside WSL, call them through WSL explicitly:
