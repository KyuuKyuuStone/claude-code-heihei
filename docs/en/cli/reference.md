...
Claude Code Heihei starts an interactive session by default. With `--print`, it can also run as a non-interactive agent in scripts, CI, or another program. The command in a source checkout is `./bin/claude-heihei`; the installed executable name depends on the installation method.

```bash
./bin/claude-heihei --help
```
...
./bin/claude-heihei "Explain how this repository starts"

# Return text and exit
./bin/claude-heihei --print "Summarize the latest commit"

# Return one complete JSON result
./bin/claude-heihei --print --output-format json "List the main modules"

# Continue the latest session in this directory
./bin/claude-heihei --continue

# Resume a session but fork it to a new session ID
./bin/claude-heihei --resume <session-id> --fork-session

# Work in a new Git worktree
./bin/claude-heihei --worktree docs-refresh
```
...
```bash
./bin/claude-heihei mcp --help
./bin/claude-heihei plugin --help
```
