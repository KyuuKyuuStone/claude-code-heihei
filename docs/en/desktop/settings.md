...
A real host shell embedded in the app, for installing plugins, skills, MCP servers, and anything else that needs a command line. The desktop app bundles `claude-heihei`, so anywhere the docs say `claude <args>` you can run `claude-heihei <args>`.
