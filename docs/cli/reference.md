...
Claude Code Heihei 默认启动交互式会话，也可以用 `--print` 作为脚本、CI 或其他程序中的非交互式 Agent。源码仓库中的命令是 `./bin/claude-heihei`；安装后的可执行文件名称以安装方式为准。
...
# 带着第一条需求进入会话
./bin/claude-heihei "解释这个仓库的启动路径"

# 返回一段文本后退出
./bin/claude-heihei --print "总结最近一次提交"

# 返回一个完整 JSON 结果
./bin/claude-heihei --print --output-format json "列出主要模块"
