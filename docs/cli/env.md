...
应用管理的 Provider 环境写入隔离的 Heihei 配置，不需要手工复制到 `~/.claude/settings.json`。当 CLI 读取到已激活的 Provider 时，会复用其认证、模型和协议设置；`openai_chat` 与 `openai_responses` Provider 会自动使用本机回环代理。

详细流程见 [第三方模型](../start/models.md)。

### `.env` 文件

源码仓库中的 `bin/claude-heihei` 会在项目根目录存在 `.env` 时加载它：
...
1. `bin/claude-heihei` 先让 Bun 加载仓库 `.env`。
2. CLI 初始化时合并已启用的用户、项目、本地、命令行和受管设置来源。
3. 已激活的 Heihei Provider 会覆盖普通 Claude 设置中的 Provider 路由变量，防止两个客户端互相污染。
