...
CLI 是 Claude Code Heihei 的内核，桌面端每个会话背后跑的都是它。如果你只想用图形界面，装桌面端就够了，见 [下载与安装](../start/install.md)；下面这些步骤是给需要终端交互、`--print` 脚本自动化，或者准备读源码、提 PR 的人看的。
...
```bash
./bin/claude-heihei
./bin/claude-heihei -p "概括当前项目的目录结构"
```
...
`./bin/claude-heihei` 只在仓库里能用。把它加进 `PATH`，就能在任何项目目录直接敲 `claude-heihei`，CLI 会自动把当前工作目录当成项目根。
...
如果 `claude-heihei` 跑在 Windows 或 Git Bash 里，而 Node、Python、uv、bun 这些工具装在 WSL 中，可以显式经 WSL 调用：
