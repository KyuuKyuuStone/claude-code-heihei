...
describe('uiStore following the system appearance', () => {
  ...
  it('freezes the theme on screen when the switch is turned off', async () => {
    ...
    useUIStore.getState().setFollowSystemTheme(false)
    expect(useUIStore.getState().theme).toBe('dark')
    expect(window.localStorage.getItem('cc-heihei-theme')).toBe('dark')
    expect(window.localStorage.getItem('cc-heihei-follow-system-theme')).toBe('0')
    teardownTheme()
  })
  ...
  it('does not revive the switch from a window whose store went stale', async () => {
    ...
    window.localStorage.setItem('cc-heihei-follow-system-theme', '0')
  ...
  it('catches up when another window changes the appearance', async () => {
    ...
    window.localStorage.setItem('cc-heihei-theme', 'celadon')
    window.localStorage.setItem('cc-heihei-light-theme', 'celadon')
    window.dispatchEvent(new StorageEvent('storage', { key: 'cc-heihei-light-theme' }))
  ...
  it('ignores storage events for unrelated keys', async () => {
    ...
    window.localStorage.setItem('cc-heihei-theme', 'celadon')
    window.dispatchEvent(new StorageEvent('storage', { key: 'cc-heihei-open-tabs' }))
...
describe('uiStore settings tab persistence', () => {
  ...
  it('hydrates the last selected Settings tab after the renderer store is recreated', async () => {
    ...
    expect(window.localStorage.getItem('cc-heihei-active-settings-tab')).toBe('general')
  ...
  it('ignores an invalid persisted Settings tab', async () => {
    window.localStorage.setItem('cc-heihei-active-settings-tab', 'not-a-settings-tab')
