...
describe('Settings > General tab', () => {
  beforeEach(() => {
    ...
    tauriDialogMock.open.mockResolvedValue('/Users/test/cc-heihei-data')
    ...
    useSettingsStore.setState({
      ...
      traceCapture: { enabled: true, storageDir: '/Users/test/.claude/cc-heihei/traces' },
  ...
  it('lets desktop users choose a custom data directory and relaunch immediately', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Custom data directory')).toHaveValue('/Users/test/cc-heihei-data')
    })
    ...
    await waitFor(() => {
      expect(useSettingsStore.getState().setAppMode).toHaveBeenCalledWith('portable', '/Users/test/cc-heihei-data')
  ...
  it('switches back to ~/.claude without deleting custom data', async () => {
    useSettingsStore.setState({
      appMode: {
        mode: 'portable',
        portableDir: '/Users/test/cc-heihei-data',
        activeConfigDir: '/Users/test/cc-heihei-data',
  ...
  it('requests native notification permission when desktop notifications are enabled', async () => {
    ...
    expect(desktopNotificationsMock.notifyDesktop).toHaveBeenCalledWith({
      title: 'Claude Code Heihei notifications are enabled',
...
describe('Settings > About tab', () => {
  beforeEach(() => {
    ...
    useUpdateStore.setState({
      ...
      releaseNotes: '# Claude Code Heihei v0.1.5\n\n- Fixed updater rendering\n- Added markdown support',
      progressPercent: 0,
      downloadedBytes: 0,
      totalBytes: null,
      error: null,
  ...
  it('renders release notes with markdown formatting', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Claude Code Heihei v0.1.5' })).toBeInTheDocument()
