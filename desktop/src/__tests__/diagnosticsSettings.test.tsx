...
describe('Settings > Diagnostics tab', () => {
  beforeEach(() => {
    ...
    diagnosticsApiMock.getStatus.mockResolvedValue({
      logDir: '/tmp/claude/cc-heihei/diagnostics',
      diagnosticsPath: '/tmp/claude/cc-heihei/diagnostics/diagnostics.jsonl',
      cliDiagnosticsPath: '/tmp/claude/cc-heihei/diagnostics/cli-diagnostics.jsonl',
      runtimeErrorsPath: '/tmp/claude/cc-heihei/diagnostics/runtime-errors.log',
      exportDir: '/tmp/claude/cc-heihei/diagnostics/exports',
      retentionDays: 7,
    ...
    diagnosticsApiMock.exportBundle.mockResolvedValue({
      bundle: {
        path: '/tmp/claude/cc-heihei/diagnostics/exports/cc-heihei-diagnostics.tar.gz',
        fileName: 'cc-heihei-diagnostics.tar.gz',
        bytes: 1024,
      },
    })
    ...
    doctorApiMock.report.mockResolvedValue({
      report: {
        ...
        items: [
          {
            id: 'cc-heihei-providers',
            label: 'Managed providers',
            kind: 'json',
            scope: 'user',
            path: '~/.claude/cc-heihei/providers.json',
            protected: true,
            exists: true,
  ...
  it('shows diagnostics status, actions, and recent events', async () => {
    ...
    expect(screen.getByText('/tmp/claude/cc-heihei/diagnostics')).toBeInTheDocument()
  ...
  it('lets a pending rebuild win over an export refresh and always releases its busy state', async () => {
    ...
    await act(async () => {
      exportRequest.resolve({
        bundle: {
          path: '/tmp/claude/cc-heihei/diagnostics/exports/race.tar.gz',
  ...
  it('keeps legacy diagnostics usable when the additive local-index endpoint is unavailable', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/cc-heihei/diagnostics')).toBeInTheDocument()
  ...
  it('exports a diagnostics bundle from the settings page', async () => {
    ...
    expect(await screen.findByText('/tmp/claude/cc-heihei/diagnostics/exports/cc-heihei-diagnostics.tar.gz')).toBeInTheDocument()
  })
  ...
  it('checks findings first and confirms before resetting only safe desktop state', async () => {
    ...
    window.localStorage.setItem('cc-heihei-chat-history', 'keep')
    ...
    expect(window.localStorage.getItem('cc-heihei-theme')).toBe('cc-heihei-theme-value')
    expect(screen.getByText('~/.claude/cc-heihei/providers.json')).toBeInTheDocument()
    ...
    expect(screen.getByText(/cc-heihei-app-zoom/)).toBeInTheDocument()
    expect(screen.getByText(/cc-heihei-ui-zoom/)).toBeInTheDocument()
    ...
    expect(window.localStorage.getItem('cc-heihei-chat-history')).toBe('keep')
    expect(screen.getByText(/Removed keys:.*cc-heihei-app-zoom/)).toBeInTheDocument()
  })
  ...
  it('clears an existing Doctor report when the active cwd changes', async () => {
    ...
    fireEvent.click(screen.getByText('Diagnostics'))
    fireEvent.click(await screen.findByRole('button', { name: /Run Doctor/i }))
    expect(await screen.findByText('~/.claude/cc-heihei/providers.json')).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(screen.queryByText('~/.claude/cc-heihei/providers.json')).not.toBeInTheDocument()
    })
  ...
  it('preserves reset results and releases reset loading when cwd changes during refresh', async () => {
    ...
    expect(await screen.findByText(/Removed keys:.*cc-heihei-app-zoom/)).toBeInTheDocument()
