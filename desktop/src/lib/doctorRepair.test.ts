...
describe('doctorRepair', () => {
  ...
  it('clears only the safe desktop UI storage keys', () => {
    ...
    window.localStorage.setItem('cc-heihei-chat-history', 'preserve')
    window.localStorage.setItem('cc-heihei-provider-config', 'preserve')
    ...
    expect(window.localStorage.getItem('cc-heihei-chat-history')).toBe('preserve')
    expect(window.localStorage.getItem('cc-heihei-provider-config')).toBe('preserve')
  })
  ...
  it('resets the appearance completely, not just the applied theme', () => {
    ...
    expect(SAFE_DOCTOR_STORAGE_KEYS).toEqual(expect.arrayContaining([
      'cc-heihei-theme',
      'cc-heihei-follow-system-theme',
      'cc-heihei-light-theme',
    ]))
  })
