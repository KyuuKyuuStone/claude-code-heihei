...
describe('desktop persistence migrations', () => {
  ...
  test('migrates legacy open-tab arrays into the current tab persistence shape', () => {
    window.localStorage.setItem('cc-heihei-open-tabs', JSON.stringify([
    ...
    expect(report.migratedKeys).toContain('cc-heihei-open-tabs')
    expect(JSON.parse(window.localStorage.getItem('cc-heihei-open-tabs') || '{}')).toEqual({
  ...
  test('filters stale session runtime selections without clearing unrelated keys', () => {
    ...
    window.localStorage.setItem('cc-heihei-session-runtime', JSON.stringify({
    ...
    expect(JSON.parse(window.localStorage.getItem('cc-heihei-session-runtime') || '{}')).toEqual({
      alsoGood: { providerId: 'openai-official', modelId: 'gpt-5.6-sol', effortLevel: 'xhigh' },
      good: { providerId: null, modelId: 'claude-sonnet' },
    })
  ...
  test('removes malformed known keys without throwing during startup', () => {
    window.localStorage.setItem('cc-heihei-open-tabs', '{"openTabs":')
    window.localStorage.setItem('cc-heihei-theme', 'sepia')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('cc-heihei-open-tabs')
    expect(report.migratedKeys).toContain('cc-heihei-theme')
    expect(window.localStorage.getItem('cc-heihei-open-tabs')).toBeNull()
    expect(window.localStorage.getItem('cc-heihei-theme')).toBeNull()
  })

  test('preserves the pure white theme as a valid persisted theme', () => {
    window.localStorage.setItem('cc-heihei-theme', 'white')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).not.toContain('cc-heihei-theme')
    expect(window.localStorage.getItem('cc-heihei-theme')).toBe('white')
  })
  ...
  test('renames the retired light theme to warm-classic instead of resetting it', () => {
    ...
    window.localStorage.setItem('cc-heihei-theme', 'light')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('cc-heihei-theme')
    expect(window.localStorage.getItem('cc-heihei-theme')).toBe('warm-classic')
  })
  ...
  test('applies the same rename to the light half of follow-the-system', () => {
    ...
    window.localStorage.setItem('cc-heihei-light-theme', 'light')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('cc-heihei-light-theme')
    expect(window.localStorage.getItem('cc-heihei-light-theme')).toBe('warm-classic')
  })

  test('preserves every palette introduced by the redesign', () => {
    for (const theme of ['white', 'paper', 'warm-classic', 'celadon', 'dark', 'ink-blue']) {
      window.localStorage.setItem('cc-heihei-theme', theme)

      const report = runDesktopPersistenceMigrations()

      expect(report.migratedKeys, `${theme} should survive startup migration`).not.toContain('cc-heihei-theme')
      expect(window.localStorage.getItem('cc-heihei-theme')).toBe(theme)
    }
  })
  ...
  test('drops a malformed follow-the-system flag rather than reading it as opted in', () => {
    ...
    window.localStorage.setItem('cc-heihei-follow-system-theme', 'yes')
    // A dark palette is not a valid light half, and vice versa.
    window.localStorage.setItem('cc-heihei-light-theme', 'ink-blue')
    window.localStorage.setItem('cc-heihei-dark-theme', 'celadon')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).toContain('cc-heihei-follow-system-theme')
    expect(report.migratedKeys).toContain('cc-heihei-light-theme')
    expect(report.migratedKeys).toContain('cc-heihei-dark-theme')
    expect(window.localStorage.getItem('cc-heihei-follow-system-theme')).toBeNull()
    expect(window.localStorage.getItem('cc-heihei-light-theme')).toBeNull()
    expect(window.localStorage.getItem('cc-heihei-dark-theme')).toBeNull()
  })

  test('preserves a valid follow-the-system flag and both ground preferences', () => {
    window.localStorage.setItem('cc-heihei-follow-system-theme', '1')
    window.localStorage.setItem('cc-heihei-light-theme', 'celadon')
    window.localStorage.setItem('cc-heihei-dark-theme', 'ink-blue')

    const report = runDesktopPersistenceMigrations()

    expect(report.migratedKeys).not.toContain('cc-heihei-follow-system-theme')
    expect(report.migratedKeys).not.toContain('cc-heihei-light-theme')
    expect(report.migratedKeys).not.toContain('cc-heihei-dark-theme')
    expect(window.localStorage.getItem('cc-heihei-follow-system-theme')).toBe('1')
    expect(window.localStorage.getItem('cc-heihei-light-theme')).toBe('celadon')
    expect(window.localStorage.getItem('cc-heihei-dark-theme')).toBe('ink-blue')
  })

  test('preserves every supported locale during startup migration', () => {
    for (const locale of ['en', 'zh', 'zh-TW', 'jp', 'kr']) {
      window.localStorage.setItem('cc-heihei-locale', locale)

      const report = runDesktopPersistenceMigrations()

      expect(report.migratedKeys).not.toContain('cc-heihei-locale')
      expect(window.localStorage.getItem('cc-heihei-locale')).toBe(locale)
    }
  })

  test('preserves valid app zoom and removes invalid app zoom values', () => {
    window.localStorage.setItem('cc-heihei-app-zoom', '1.2')

    const validReport = runDesktopPersistenceMigrations()

    expect(validReport.migratedKeys).not.toContain('cc-heihei-app-zoom')
    expect(window.localStorage.getItem('cc-heihei-app-zoom')).toBe('1.2')

    window.localStorage.setItem('cc-heihei-app-zoom', '4')

    const invalidReport = runDesktopPersistenceMigrations()

    expect(invalidReport.migratedKeys).toContain('cc-heihei-app-zoom')
    expect(window.localStorage.getItem('cc-heihei-app-zoom')).toBeNull()
  })

  test('migrates the legacy UI zoom key into app zoom storage', () => {
    window.localStorage.setItem('cc-heihei-ui-zoom', '1.25')
    ...
    expect(window.localStorage.getItem('cc-heihei-app-zoom')).toBe('1.25')
    expect(window.localStorage.getItem('cc-heihei-ui-zoom')).toBeNull()
  })
