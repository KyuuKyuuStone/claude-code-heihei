...
const traceList: TraceSessionList = {
  total: 1,
  storageDir: '/tmp/cc-heihei/traces',
  settings: {
    enabled: true,
    storageDir: '/tmp/cc-heihei/traces',
  },
...
describe('TraceList', () => {
  ...
  it('renders rows with title, model chips, failure count and metrics', async () => {
    ...
    expect(screen.getByText('/tmp/cc-heihei/traces')).toBeInTheDocument()
  ...
  it('announces a failed load and retries from inside the error', async () => {
    ...
    expect(await screen.findByText('/tmp/cc-heihei/traces')).toBeInTheDocument()
  ...
  it('shows the paused badge when capture is disabled', async () => {
    vi.mocked(tracesApi.list).mockResolvedValue({
      ...traceList,
      settings: { enabled: false, storageDir: '/tmp/cc-heihei/traces' },
    })
