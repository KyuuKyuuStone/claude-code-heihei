...
afterEach(() => {
  ...
  useHeiheiOAuthStore.setState(useHeiheiOAuthStore.getInitialState(), true)
  useHeiheiOpenAIOAuthStore.setState(useHeiheiOpenAIOAuthStore.getInitialState(), true)
  useHeiheiGrokOAuthStore.setState(useHeiheiGrokOAuthStore.getInitialState(), true)
})

beforeEach(() => {
  useHeiheiOAuthStore.setState({ fetchStatus: async () => {} })
  useHeiheiOpenAIOAuthStore.setState({ fetchStatus: async () => {} })
  useHeiheiGrokOAuthStore.setState({ fetchStatus: async () => {} })
})
...
describe('ModelSelector', () => {
  ...
  it('does not query official OAuth status when mounted', () => {
    ...
    useHeiheiOAuthStore.setState({ fetchStatus: fetchClaudeStatus })
    useHeiheiOpenAIOAuthStore.setState({ fetchStatus: fetchOpenAIStatus })
    useSettingsStore.setState({
      locale: 'en',
  ...
  it('hides official provider sections when OAuth is not logged in', async () => {
    useHeiheiOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useHeiheiOpenAIOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useHeiheiGrokOAuthStore.setState({ status: { loggedIn: false }, fetchStatus: async () => {} })
    useSettingsStore.setState({
      locale: 'en',
      availableModels: MODELS,
      currentModel: MODELS[0],
