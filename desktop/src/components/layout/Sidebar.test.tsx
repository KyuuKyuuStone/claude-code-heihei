...
describe('Sidebar', () => {
  ...
  // The header used to render both "Claude Code Heihei" and "cc-heihei" and hide
  ...
  it('renders one wordmark and it is the short one', () => {
    ...
    expect(region).toHaveTextContent('cc-heihei')
  ...
  it('does not restore a hidden Windows drive root when creating a child project session', async () => {
    ...
    useSessionStore.setState({
      sessions: [
        makeSession('child-1', 'Child Session', 'D:\\workspace\\code\\cc-heihei', now),
      ],
    })
    useTabStore.setState({
    ...
    await waitFor(() => {
      expect(createSession).toHaveBeenCalledWith('D:\\workspace\\code\\cc-heihei')
    })
