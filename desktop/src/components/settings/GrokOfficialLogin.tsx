...
import { heiheiGrokOAuthApi } from '../../api/heiheiGrokOAuth'
...
export function GrokOfficialLogin() {
  ...
  useEffect(() => {
    ...
    void getDesktopHost().shell.open(heiheiGrokOAuthApi.successUrl()).catch((err) => {
  ...
  const handleCopyAuthorizeUrl = async () => {
    ...
    if (await copyTextToClipboard(manualAuthorizeUrl)) {
      setManualAuthorizeUrl(null)
      setIsAwaitingAuthorization(true)
      useHeiheiGrokOAuthStore.setState({ error: null })
      startPolling()
    } else {
      useHeiheiGrokOAuthStore.setState({
