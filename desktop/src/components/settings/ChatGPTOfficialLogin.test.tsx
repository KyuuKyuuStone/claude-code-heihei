...
const initialOAuthState = useHeiheiOpenAIOAuthStore.getState()
...
describe('ChatGPTOfficialLogin', () => {
  ...
  afterEach(() => {
    act(() => {
      ...
      useHeiheiOpenAIOAuthStore.setState(initialOAuthState)
    })
    vi.useRealTimers()
    cleanup()
    vi.restoreAllMocks()
  ...
  it('keeps an actionable authorization link when shell open fails', async () => {
    ...
    expect(useHeiheiOpenAIOAuthStore.getState().error).toBeNull()
