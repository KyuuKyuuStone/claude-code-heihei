  ...
  it('does not duplicate the branch in the selected project chip', async () => {
    vi.mocked(sessionsApi.getRecentProjects).mockResolvedValue({
      projects: [{
        ...
        projectName: 'project',
        repoName: 'cc-heihei/OpenCutSkill',
        branch: 'main',
        isGit: true,
    ...
    const trigger = await waitFor(() => screen.getAllByRole('button', { name: /cc-heihei\/OpenCutSkill/ })[0])
    expect(trigger).toHaveTextContent('cc-heihei/OpenCutSkill')
