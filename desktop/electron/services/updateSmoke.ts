...
export type UpdateSmokeEnv = {
  CC_HEIHEI_ELECTRON_UPDATE_SMOKE_VERSION?: string
  CC_HEIHEI_ELECTRON_UPDATE_SMOKE_BODY?: string
  CC_HEIHEI_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES?: string
  CC_HEIHEI_ELECTRON_UPDATE_SMOKE_LOG?: string
}
...
export function parseUpdateSmokeEnv(env: UpdateSmokeEnv): UpdateSmokeConfig | null {
  const version = env.CC_HEIHEI_ELECTRON_UPDATE_SMOKE_VERSION?.trim()
  if (!version) return null
  return {
    version,
    body: env.CC_HEIHEI_ELECTRON_UPDATE_SMOKE_BODY ?? 'Electron update smoke release',
    totalBytes: parsePositiveInteger(env.CC_HEIHEI_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES, 100),
    logPath: env.CC_HEIHEI_ELECTRON_UPDATE_SMOKE_LOG,
  }
}
