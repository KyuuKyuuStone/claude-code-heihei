...
function tempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cc-heihei-app-mode-'))
  tempDirs.push(dir)
  return dir
}

function app(root = tempDir()): AppModeAppLike & { root: string } {
  const exe = path.join(root, 'install', 'Claude Code Heihei')
...
describe('Electron app mode service', () => {
  ...
  it('drops inherited app-managed env so switching back to ~/.claude survives relaunch', () => {
    ...
    expect(env.CC_HEIHEI_APP_PORTABLE_DIR).toBeUndefined()
