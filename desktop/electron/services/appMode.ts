...
function assertWritableDataDir(configDir: string): void {
  try {
    fs.mkdirSync(configDir, { recursive: true })
    const probeDir = fs.mkdtempSync(path.join(configDir, '.cc-heihei-write-test-'))
    try {
...
function externallyControlled(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.CLAUDE_CONFIG_DIR && env.CC_HEIHEI_APP_PORTABLE_DIR !== '1')
}
...
export function applyStartupPortableMode(
  ...
  if (env.CC_HEIHEI_APP_PORTABLE_DIR === '1') {
    delete env.CLAUDE_CONFIG_DIR
    delete env.CC_HEIHEI_APP_PORTABLE_DIR
  ...
  env.CC_HEIHEI_APP_PORTABLE_DIR = '1'
...
export function getAppMode(
  ...
  if (customDir) {
    return {
      ...
      configDirSource: envConfigDir && env.CC_HEIHEI_APP_PORTABLE_DIR !== '1'
        ? 'environment'
        : 'portable',
    }
  }
