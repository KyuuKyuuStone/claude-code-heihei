...
describe('Electron update smoke updater', () => {
  it('stays disabled unless a smoke version is configured', () => {
    ...
    expect(parseUpdateSmokeEnv({ CC_HEIHEI_ELECTRON_UPDATE_SMOKE_VERSION: '  ' })).toBeNull()
  })

  it('drives the real Electron updater service contract and logs the install signal', async () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'cc-heihei-update-smoke-'))
    const logPath = join(tempDir, 'update-smoke.jsonl')
    try {
      const updater = createUpdateSmokeUpdaterFromEnv({
        CC_HEIHEI_ELECTRON_UPDATE_SMOKE_VERSION: '9.9.9-smoke',
        CC_HEIHEI_ELECTRON_UPDATE_SMOKE_BODY: 'Smoke notes',
        CC_HEIHEI_ELECTRON_UPDATE_SMOKE_TOTAL_BYTES: '200',
        CC_HEIHEI_ELECTRON_UPDATE_SMOKE_LOG: logPath,
      })
