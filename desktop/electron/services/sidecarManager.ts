...
export const SYSTEM_PROXY_BRIDGE_ENV = 'CC_HEIHEI_SYSTEM_PROXY_URL'
export const SYSTEM_PROXY_ERROR_ENV = 'CC_HEIHEI_SYSTEM_PROXY_ERROR'
...
function ensurePrivateHostDiagnosticsDirectory(directory: string): void {
  ...
  const rootBoundary = path.basename(directory) === 'diagnostics' &&
      path.basename(parent) === 'cc-heihei'
    ? path.dirname(parent)
    : parent
