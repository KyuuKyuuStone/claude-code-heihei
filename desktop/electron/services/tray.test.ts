  ...
  it('installs tray handlers that show the app, quit explicitly, and dispose cleanly', async () => {
    ...
    try {
      ...
      const controller = await installTray({
        app: { name: 'Claude Code Heihei' } as never,
        desktopRoot: root,
        show,
        quit,
        electronRuntime: {
      ...
      expect(trayMocks.tray.setToolTip).toHaveBeenCalledWith('Claude Code Heihei')
      ...
      expect(template.map(item => item.label ?? item.type)).toEqual([
        'Show Claude Code Heihei',
        'separator',
        'Quit Claude Code Heihei',
      ])
