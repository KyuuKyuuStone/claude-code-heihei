...
export function logNotificationSmokeRendererAck(env: NodeJS.ProcessEnv, payload: unknown): boolean {
  const logPath = env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_LOG?.trim()
...
export function scheduleNotificationSmoke({
  ...
  const sessionId = env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_SESSION_ID?.trim()
  if (!sessionId) return false

  const title = env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_TITLE?.trim() || 'Claude Code Heihei notification smoke'
  const body = env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_BODY?.trim() || 'Click to return to the target session.'
  const delayMs = parseNotificationSmokeDelay(env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_DELAY_MS)
  const triggerSyntheticAction = shouldTriggerSyntheticAction(env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_TRIGGER_ACTION)
  const logPath = env.CC_HEIHEI_ELECTRON_NOTIFICATION_SMOKE_LOG?.trim()
