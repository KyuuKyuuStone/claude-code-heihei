...
fn get_app_mode(app: AppHandle) -> serde_json::Value {
    ...
    let config_dir_source = if env_config_dir.is_some() {
        if std::env::var_os("CC_HEIHEI_APP_PORTABLE_DIR").is_some() {
            "portable"
        } else {
            "environment"
        }
...
fn window_state_path(app: &AppHandle) -> Option<PathBuf> {
    ...
    // %APPDATA%\com.claude-code-heihei.desktop\.
...
fn server_state_path() -> Option<PathBuf> {
    // Lives next to cc-heihei/settings.json (CLAUDE_CONFIG_DIR or ~/.claude) so
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Claude Code Heihei")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Claude Code Heihei")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Claude Code Heihei")
        .menu(&menu)
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Claude Code Heihei").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Claude Code Heihei")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()
