...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/cc-heihei-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-cc-heihei-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-cc-heihei-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/cc-heihei" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'cc-heihei'), { recursive: true })
writeFileSync(join(configDir, 'cc-heihei', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('cc-heihei-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('cc-heihei-session-runtime', JSON.stringify({
