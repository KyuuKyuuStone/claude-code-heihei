...
function readCliLauncher() {
  return readFileSync(
    path.resolve(import.meta.dirname, '../../bin/claude-heihei'),
    'utf8',
  )
}
...
function controlledSidecarEnvironment(
  ...
  return {
    ...
    CC_HEIHEI_LOCAL_ACCESS_TOKEN: localAccessToken,
    NODE_ENV: 'test',
...
describe('build-sidecars Windows x64 target mapping', () => {
  ...
  it('bounds a sidecar response with headers but a never-ending body and still cleans up', async () => {
    ...
    const originalEnvironment = {
      ...
      CC_HEIHEI_LOCAL_INDEX: process.env.CC_HEIHEI_LOCAL_INDEX,
      CC_HEIHEI_LOCAL_ACCESS_TOKEN: process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN,
    }
    const rootDir = await mkdtemp(joinPath(tmpdir(), 'cc-heihei-hung-sidecar-smoke-'))
    ...
    expect(process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN).toBe(
      originalEnvironment.CC_HEIHEI_LOCAL_ACCESS_TOKEN,
    )
  })
})

const compiledSidecarSmokeEnabled =
  process.env.CC_HEIHEI_RUN_COMPILED_SIDECAR_SMOKE === '1'
const configuredCompiledSidecarStarts = Number.parseInt(
  process.env.CC_HEIHEI_COMPILED_SIDECAR_SMOKE_STARTS ?? '',
  10,
)
...
describe.skipIf(!compiledSidecarSmokeEnabled)('compiled sidecar local-index smoke', () => {
  it('uses SQLite by default, serves one indexed session, and reopens the database', async () => {
    ...
    const rootDir = await mkdtemp(joinPath(tmpdir(), 'cc-heihei-compiled-sidecar-smoke-'))
