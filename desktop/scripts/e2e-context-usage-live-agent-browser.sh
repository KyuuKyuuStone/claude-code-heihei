...
SESSION_NAME="cc-heihei-context-live-${RUN_ID}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d "/tmp/cc-heihei-context-live-${RUN_ID}-XXXX")}"
...
"${AB[@]}" eval "localStorage.setItem('cc-heihei-locale', 'en'); localStorage.setItem('cc-heihei-open-tabs', '${TAB_STATE}'); location.reload();" >>"${BROWSER_LOG}" 2>&1
