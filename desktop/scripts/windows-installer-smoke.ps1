...
$installers = @(Get-ChildItem -LiteralPath $resolvedArtifactsDir -File |
  Where-Object { $_.Name -like "Claude-Code-Heihei-*-win-$Arch.exe" })
if ($installers.Count -ne 1) {
  throw "Expected exactly one Windows $Arch installer in $resolvedArtifactsDir, found $($installers.Count)."
}
$installer = $installers[0].FullName

$testRoot = Join-Path ([IO.Path]::GetTempPath()) "cc-heihei-installer-smoke-$([Guid]::NewGuid().ToString('N'))"
$installDir = Join-Path $testRoot '中文 安装目录\Claude Code Heihei'
...
$appExe = Join-Path $installDir 'Claude Code Heihei.exe'
$uninstaller = Join-Path $installDir 'Uninstall Claude Code Heihei.exe'
...
$savedEnvironment = @{}
foreach ($name in @('APPDATA', 'LOCALAPPDATA', 'USERPROFILE', 'CLAUDE_CONFIG_DIR', 'CC_HEIHEI_APP_PORTABLE_DIR', 'COMPLUS_Version')) {
...
# unavailable (see CcHeiheiFindInstallProcess's findstr list). Without a CLR the
...
function Invoke-InstalledApplicationSmoke {
  ...
  $hostDiagnostics = Join-Path $configDir 'cc-heihei\diagnostics\electron-host.log'
  $smokeEnvironmentNames = @(
    'CLAUDE_CONFIG_DIR',
    'CC_HEIHEI_ELECTRON_WINDOW_SMOKE_LOG',
    'HOME',
    'CI',
    'NO_PROXY',
    'no_proxy'
  ...
  try {
    ...
    $env:CC_HEIHEI_ELECTRON_WINDOW_SMOKE_LOG = $windowLog
        ...
        try {
          Invoke-ProcessHelperCleanup `
            ...
            -Arguments @(
              ...
              '-ProcessName', 'Claude Code Heihei.exe',
              '-Action', 'KillForce',
...
function Invoke-LegacyRecoveryDiagnostic {
  ...
  $arguments = @(
    ...
    $installDir,
    '-UserDataDir',
    (Join-Path $appData 'Claude Code Heihei'),
    '-RecoveryRoot',
    (Join-Path $userProfile 'Claude Code Heihei Data\Recovered'),
    '-ProcessName',
    'Claude Code Heihei.exe',
...
try {
  ...
  Remove-Item Env:CC_HEIHEI_APP_PORTABLE_DIR -ErrorAction SilentlyContinue
  ...
  $siblingProbe = Join-Path $siblingDir 'Claude Code Heihei.exe'
