...
- `composite/BrandSeal` — 品牌标记 cc-heihei（原 app-icon 的矢量重建），`sm`24 / `md`32 / `lg`38 / `xl`80，随尺寸减件（星芒只在 `xl`，光标到 `sm` 去掉）。
...
`/Users/nanmi/Downloads/design_handoff_cc_heihei_redesign/` 下的 `CC Heihei.dc.html` 是高保真交互原型，README.md 是像素级规格。原型用浏览器直接打开可交互。
