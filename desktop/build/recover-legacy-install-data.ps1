...
param(
  ...
  [string]$UserDataDir = '',
  [string]$RecoveryRoot = '',
  [string]$ProcessName = 'Claude Code Heihei.exe',
  [string]$ActiveConfigDir = $env:CLAUDE_CONFIG_DIR,
  [string]$ActiveConfigManaged = $env:CC_HEIHEI_APP_PORTABLE_DIR,
...
function Test-LegacyPortableData {
  ...
  foreach ($childDir in @('Cache', 'EBWebView', 'projects', 'skills', 'plugins', 'cowork_plugins', 'cc-heihei')) {
    ...
    foreach ($installDir in $InstallDirs) {
      if (Test-PathMayBeDeleted -InstallDir $installDir -Candidate $active) {
        ...
        if ($ActiveConfigManaged -ne '1') {
          throw "Active CLAUDE_CONFIG_DIR is managed outside Claude Code Heihei and points inside an application install directory. Move or remove that environment variable before upgrading: $active"
        }
...
function Run-SelfTest {
  $testRoot = Join-Path ([IO.Path]::GetTempPath()) "cc-heihei-storage-recovery-$([Guid]::NewGuid().ToString('N'))"
  ...
  try {
    ...
    try {
      ...
      $unmanagedFailed = $_.Exception.Message.Contains('managed outside Claude Code Heihei')
    }
