...
function getCcHeiheiDir(): string {
  const configDir =
    process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude')
  return path.join(configDir, 'cc-heihei')
}

function getFileBackedStorageMarkerPath(): string {
  return path.join(getCcHeiheiDir(), FILE_BACKED_STORAGE_MARKER_FILE)
}

function markFileBackedStorageUsed(): void {
  try {
    fs.mkdirSync(getCcHeiheiDir(), { recursive: true })
