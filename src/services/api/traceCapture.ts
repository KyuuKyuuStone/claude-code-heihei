...
export function shouldCaptureApiTrace(): boolean {
  if (isEnvDefinedFalsy(process.env.CC_HEIHEI_TRACE_API_CALLS)) return false
  if (isEnvTruthy(process.env.CC_HEIHEI_TRACE_API_CALLS)) return true
...
export function getTraceStorageDir(): string {
  return join(getClaudeConfigHomeDir(), 'cc-heihei', 'traces')
}
...
function currentTraceScopeContext(): TraceScopeContext {
  ...
  return {
    scope,
    storageDir: join(scope, 'cc-heihei', 'traces'),
    target: {
      path: join(scope, 'cc-heihei', 'db', 'trace-index-v1.sqlite'),
      scope,
    },
  }
}
...
function getManagedSettingsPath(scope = getClaudeConfigHomeDir()): string {
  return join(scope, 'cc-heihei', 'settings.json')
}
