...
describe('getAnthropicClient', () => {
  test('selects the isolated Grok transport with a dummy SDK key', async () => {
    ...
    const previous = {
      marker: process.env.CC_HEIHEI_GROK_OAUTH_PROVIDER,
      tokenFile: process.env.GROK_OAUTH_FILE,
      configDir: process.env.CLAUDE_CONFIG_DIR,
    }
    process.env.CC_HEIHEI_GROK_OAUTH_PROVIDER = '1'
    ...
    try {
      ...
      if (previous.marker === undefined) delete process.env.CC_HEIHEI_GROK_OAUTH_PROVIDER
      else process.env.CC_HEIHEI_GROK_OAUTH_PROVIDER = previous.marker
  ...
  test('authenticates the host-managed desktop provider proxy with the local access token', async () => {
    ...
    const previous = {
      ...
      localAccessToken: process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN,
    ...
    try {
      ...
      for (const [envKey, value] of [
        ...
        ['CC_HEIHEI_LOCAL_ACCESS_TOKEN', previous.localAccessToken],
