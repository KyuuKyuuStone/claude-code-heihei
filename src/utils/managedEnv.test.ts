...
describe('managedEnv', () => {
  ...
  afterEach(async () => {
    ...
    restoreEnv('CC_HEIHEI_LOCAL_ACCESS_TOKEN')
  ...
  test('starts a standalone provider proxy for CLI-only OpenAI-compatible providers', async () => {
    await writeJson(path.join(tmpDir, 'cc-heihei', 'providers.json'), {
  ...
  test('does not let settings replace host-owned provider routing credentials', async () => {
    await writeJson(path.join(tmpDir, 'cc-heihei', 'settings.json'), {
      env: {
        CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST: '0',
        CC_HEIHEI_LOCAL_ACCESS_TOKEN: 'stale-settings-token',
      },
    })
    ...
    expect(process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN).toBe('desktop-local-secret')
  })
})
