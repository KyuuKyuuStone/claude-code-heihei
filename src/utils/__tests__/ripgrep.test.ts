...
const originalExplicitPath = process.env[CC_HEIHEI_RIPGREP_PATH_ENV]

afterEach(async () => {
  await Promise.all(tempFiles.splice(0).map(path => rm(path, { force: true })))
  if (originalExplicitPath === undefined) {
    delete process.env[CC_HEIHEI_RIPGREP_PATH_ENV]
  } else {
    process.env[CC_HEIHEI_RIPGREP_PATH_ENV] = originalExplicitPath
  }
  resetRipgrepStateForTests()
})
...
describe('isUsableBuiltinRipgrepPath', () => {
  ...
  test('rejects missing paths', () => {
    expect(
      isUsableBuiltinRipgrepPath(join(tmpdir(), 'missing-cc-heihei-rg')),
    ).toBe(false)
  })

  test('accepts real filesystem paths', async () => {
    const filePath = join(tmpdir(), `cc-heihei-rg-${Date.now()}`)
...
describe('packaged ripgrep resolution', () => {
  ...
  test('prefers an explicit packaged executable over PATH lookup', async () => {
    const filePath = join(tmpdir(), `cc-heihei-explicit-rg-${Date.now()}`)
    await writeFile(filePath, '')
    tempFiles.push(filePath)
    process.env[CC_HEIHEI_RIPGREP_PATH_ENV] = filePath
    resetRipgrepStateForTests()
