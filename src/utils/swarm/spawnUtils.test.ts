...
const originalOpenAIEffort = process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT
...
afterEach(() => {
  ...
  restoreEnv('CC_HEIHEI_OPENAI_REASONING_EFFORT', originalOpenAIEffort)
...
describe('buildInheritedEnvVars', () => {
  ...
  test('forwards the Desktop OpenAI session effort to tmux teammates', () => {
    process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT = 'max'

    expect(buildInheritedEnvVars()).toContain(
      'CC_HEIHEI_OPENAI_REASONING_EFFORT=max',
    )
  })
  ...
  test('forwards thinking controls and provider model capabilities', () => {
    ...
    process.env.CC_HEIHEI_SEND_DISABLED_THINKING = '1'
