/**
 * Unit tests for HeiheiOAuthService — heihei 自管 OAuth 的核心 service 层。
 */
...
} from '../services/heiheiOAuthService.js'
...
let service: HeiheiOAuthService

async function setup() {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'heihei-oauth-test-'))
  originalConfigDir = process.env.CLAUDE_CONFIG_DIR
  originalSystemProxyUrl = process.env[SYSTEM_PROXY_URL_ENV]
  originalFetch = globalThis.fetch
  delete process.env[SYSTEM_PROXY_URL_ENV]
  process.env.CLAUDE_CONFIG_DIR = tmpDir
  service = new HeiheiOAuthService()
}
...
describe('HeiheiOAuthService — file storage', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('saveTokens writes file with 0600 permissions', async () => {
    ...
    const oauthPath = path.join(tmpDir, 'cc-heihei', 'oauth.json')
...
describe('HeiheiOAuthService — session management', () => {
  beforeEach(setup)
  afterEach(teardown)
...
describe('HeiheiOAuthService — ensureFreshAccessToken', () => {
  beforeEach(setup)
  afterEach(teardown)
