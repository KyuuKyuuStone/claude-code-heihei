...
describe('provider settings isolation', () => {
  ...
  async function readCcHeiheiSettings(): Promise<Record<string, unknown>> {
  ...
  test('activating a provider writes only cc-heihei/settings.json', async () => {
    ...
    const settings = await readCcHeiheiSettings()
  ...
  test('switching providers replaces managed env without creating settings.json', async () => {
    ...
    await service.activateProvider(minimax.id)
    let settings = await readCcHeiheiSettings()
    ...
    settings = await readCcHeiheiSettings()
  ...
  test('activation preserves unrelated cc-heihei settings and env', async () => {
  ...
  test('providers.json and cc-heihei/settings.json stay isolated from Claude settings.json', async () => {
    await fs.writeFile(
    ...
    const heihei = await readCcHeiheiSettings()
    const env = heihei.env as Record<string, string>
    expect(env.ANTHROPIC_BASE_URL).toBe('https://api.minimaxi.com/anthropic')
    expect(env.ANTHROPIC_AUTH_TOKEN).toBe('sk-heihei-key')
    expect(env.ANTHROPIC_API_KEY).toBe('')
  })
})
