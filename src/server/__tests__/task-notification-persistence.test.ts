...
describe('background task notification persistence', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cc-heihei-task-notification-'))
  ...
  it('restores a terminal Agent notification that was not forwarded into transcript history', async () => {
    ...
    expect(await fs.readFile(transcriptPath, 'utf8')).toContain('"type":"cc-heihei-task-notification"')
  })
