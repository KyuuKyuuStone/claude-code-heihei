...
let service: HeiheiGrokOAuthService
...
beforeEach(async () => {
  tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'heihei-grok-oauth-test-'))
  previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  process.env.CLAUDE_CONFIG_DIR = tempDir
  resetSettingsCache()
  previousFetch = globalThis.fetch
  service = new HeiheiGrokOAuthService()
})
...
describe('HeiheiGrokOAuthService', () => {
  test('starts a random 127.0.0.1 PKCE callback and persists exchanged tokens', async () => {
    ...
    if (process.platform !== 'win32') {
      expect((await fs.stat(getHeiheiGrokOAuthFilePath())).mode & 0o777).toBe(0o600)
    }
  })
