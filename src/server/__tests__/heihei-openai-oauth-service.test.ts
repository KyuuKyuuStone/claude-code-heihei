/**
 * Unit tests for HeiheiOpenAIOAuthService — heihei 自管 OpenAI OAuth 的核心 service 层。
 */
...
import {
  HeiheiOpenAIOAuthService,
  getHeiheiOpenAIOAuthFilePath,
  type StoredOpenAIOAuthTokens,
} from '../services/heiheiOpenAIOAuthService.js'
import { resetSettingsCache } from '../../utils/settings/settingsCache.js'

let tmpDir: string
let originalConfigDir: string | undefined
let service: HeiheiOpenAIOAuthService
...
async function setup() {
  tmpDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'heihei-openai-oauth-test-'),
  )
  originalConfigDir = process.env.CLAUDE_CONFIG_DIR
  process.env.CLAUDE_CONFIG_DIR = tmpDir
  resetSettingsCache()
  callbackPort = await getFreePort()
  service = new HeiheiOpenAIOAuthService({ callbackPort })
}
...
async function teardown() {
  ...
  restoreEnv('CC_HEIHEI_SYSTEM_PROXY_URL', originalSystemProxyUrl)
...
describe('HeiheiOpenAIOAuthService — file storage', () => {
  beforeEach(setup)
  afterEach(teardown)
  ...
  test('saveTokens writes file with 0600 permissions', async () => {
    ...
    const oauthPath = getHeiheiOpenAIOAuthFilePath()
...
describe('HeiheiOpenAIOAuthService — session management', () => {
  beforeEach(setup)
  afterEach(teardown)
...
describe('HeiheiOpenAIOAuthService — ensureFreshAccessToken', () => {
  beforeEach(setup)
  afterEach(teardown)
