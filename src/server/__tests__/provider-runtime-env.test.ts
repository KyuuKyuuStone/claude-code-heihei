...
describe('providerRuntimeEnv', () => {
  ...
  test('normalizes and preserves Grok Official as the active runtime provider', async () => {
    ...
    const env = mergeActiveProviderManagedEnv(
      {
        CC_HEIHEI_OPENAI_OAUTH_PROVIDER: '1',
    ...
    expect(env).toMatchObject({
      CC_HEIHEI_GROK_OAUTH_PROVIDER: '1',
      GROK_OAUTH_FILE: path.join(tmpDir, 'cc-heihei', 'grok-oauth.json'),
  ...
  // cc-heihei/settings.json. It is built by unioning every preset's defaultEnv keys, so
