...
describe('ConversationService', () => {
  ...
  beforeEach(async () => {
    ...
    originalLocalAccessToken = process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN
    ...
    originalTraceProviderId = process.env.CC_HEIHEI_TRACE_PROVIDER_ID
    originalTraceProviderName = process.env.CC_HEIHEI_TRACE_PROVIDER_NAME
    originalTraceProviderFormat = process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT
    ...
    delete process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN
    ...
    delete process.env.CC_HEIHEI_TRACE_PROVIDER_ID
  ...
  afterEach(async () => {
    ...
    if (originalLocalAccessToken === undefined) delete process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN
    else process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN = originalLocalAccessToken
    ...
    if (originalTraceApiCalls === undefined) delete process.env.CC_HEIHEI_TRACE_API_CALLS
    else process.env.CC_HEIHEI_TRACE_API_CALLS = originalTraceApiCalls

    if (originalTraceProviderId === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_ID
    else process.env.CC_HEIHEI_TRACE_PROVIDER_ID = originalTraceProviderId

    if (originalTraceProviderName === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_NAME
    else process.env.CC_HEIHEI_TRACE_PROVIDER_NAME = originalTraceProviderName

    if (originalTraceProviderFormat === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT
    else process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT = originalTraceProviderFormat
  ...
  test('keeps inherited provider env when no desktop provider config exists', async () => {
    ...
    const env = (await service.buildChildEnv('D:\\workspace\\code\\myself_code\\cc-heihei')) as Record<string, string>
    ...
    expect(env.CLAUDE_CODE_DIAGNOSTICS_FILE).toBe(path.join(tmpDir, 'cc-heihei', 'diagnostics', 'cli-diagnostics.jsonl'))
    expect(env.CLAUDE_COWORK_MEMORY_PATH_OVERRIDE).toBe(
      `${path.join(tmpDir, 'projects', 'D--workspace-code-myself-code-cc-heihei', 'memory')}${path.sep}`,
    )
  ...
  test('omits CLI diagnostics when its managed directory resolves through a symlink', async () => {
    ...
    await fs.mkdir(path.join(tmpDir, 'cc-heihei'), { recursive: true })
    await fs.rm(path.join(tmpDir, 'cc-heihei'), { recursive: true, force: true })
    await fs.symlink(unrelatedDir, path.join(tmpDir, 'cc-heihei'), 'dir')
  ...
  test('buildChildEnv pins desktop memory to the current sanitized project directory', async () => {
    const service = new ConversationService() as any
    const workDir = path.join(tmpDir, 'workspace', 'myself_code', 'claude-code-heihei')
  ...
  test.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      delete process.env.CC_HEIHEI_DISABLE_TERMINAL_SHELL_ENV
  ...
  test('strips inherited provider env when desktop provider config exists', async () => {
    const ccHeiheiDir = path.join(tmpDir, 'cc-heihei')
    await fs.mkdir(ccHeiheiDir, { recursive: true })
    await fs.writeFile(
      path.join(ccHeiheiDir, 'providers.json'),
  ...
  test('buildChildEnv routes system mode through the host-managed dynamic bridge', async () => {
    const originalBridgeUrl = process.env.CC_HEIHEI_SYSTEM_PROXY_URL
    process.env.CC_HEIHEI_SYSTEM_PROXY_URL = 'http://127.0.0.1:17890'
    await fs.writeFile(
      path.join(tmpDir, 'settings.json'),
      JSON.stringify({
        network: {
    ...
    try {
      ...
      if (originalBridgeUrl === undefined) delete process.env.CC_HEIHEI_SYSTEM_PROXY_URL
      else process.env.CC_HEIHEI_SYSTEM_PROXY_URL = originalBridgeUrl
    }
  })
  ...
  test('buildChildEnv injects CLAUDE_CODE_OAUTH_TOKEN when official mode + heihei oauth token exists', async () => {
    const ccHeiheiDir = path.join(tmpDir, 'cc-heihei')
    await fs.mkdir(ccHeiheiDir, { recursive: true })
    await fs.writeFile(
      path.join(ccHeiheiDir, 'settings.json'),
      JSON.stringify({ env: {} }),
      'utf-8',
    )

    const { heiheiOAuthService } = await import('../services/heiheiOAuthService.js')
    await heiheiOAuthService.saveTokens({
      accessToken: 'heihei-fresh-token',
      refreshToken: 'heihei-refresh-xxx',
    ...
    expect(env.CLAUDE_CODE_OAUTH_TOKEN).toBe('heihei-fresh-token')
  })
  ...
  test('sendMessage does not resend network env when the system bridge fingerprint is unchanged', async () => {
    const originalBridgeUrl = process.env.CC_HEIHEI_SYSTEM_PROXY_URL
    process.env.CC_HEIHEI_SYSTEM_PROXY_URL = 'http://127.0.0.1:17893'
    try {
      await fs.writeFile(
        path.join(tmpDir, 'settings.json'),
        JSON.stringify({ network: { proxy: { mode: 'system', url: '' } } }),
  ...
  test('buildChildEnv does NOT inject CLAUDE_CODE_OAUTH_TOKEN when not official mode', async () => {
    ...
    const { heiheiOAuthService } = await import('../services/heiheiOAuthService.js')
    await heiheiOAuthService.saveTokens({
      accessToken: 'heihei-token-should-not-be-used',
      refreshToken: null,
      expiresAt: null,
      scopes: [],
  ...
  test('buildChildEnv injects explicit provider runtime env for session-scoped providers', async () => {
    process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN = 'desktop-local-secret'
    ...
    expect(env.CC_HEIHEI_LOCAL_ACCESS_TOKEN).toBe('desktop-local-secret')
    expect(env.CLAUDE_CODE_ATTRIBUTION_HEADER).toBe('0')
    expect(env.CC_HEIHEI_TRANSCRIPT_ENTRYPOINT).toBe('claude-desktop')
    expect(env.CLAUDE_CODE_ENTRYPOINT).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_ID).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_NAME).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_FORMAT).toBeUndefined()
  })
  ...
  test('buildChildEnv injects trace provider metadata for desktop sdk session-scoped providers', async () => {
    ...
    expect(env.CC_HEIHEI_TRACE_API_CALLS).toBe('1')
    expect(env.CC_HEIHEI_TRACE_PROVIDER_ID).toBe(provider.id)
    expect(env.CC_HEIHEI_TRACE_PROVIDER_NAME).toBe('Traceable Provider')
    expect(env.CC_HEIHEI_TRACE_PROVIDER_FORMAT).toBe('anthropic')
  })
  ...
  test('buildChildEnv does not inject trace env when managed trace capture is disabled', async () => {
    ...
    expect(env.CC_HEIHEI_TRACE_API_CALLS).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_ID).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_NAME).toBeUndefined()
    expect(env.CC_HEIHEI_TRACE_PROVIDER_FORMAT).toBeUndefined()
  })
  ...
  test('buildChildEnv injects ChatGPT Official runtime env for session-scoped provider selection', async () => {
    ...
    expect(env.CC_HEIHEI_OPENAI_OAUTH_PROVIDER).toBe('1')
    expect(env.OPENAI_CODEX_OAUTH_FILE).toBe(
      path.join(tmpDir, 'cc-heihei', 'openai-oauth.json'),
    )
  ...
  test('buildChildEnv injects isolated Grok Official runtime env for session-scoped selection', async () => {
    ...
    expect(env.CC_HEIHEI_GROK_OAUTH_PROVIDER).toBe('1')
    expect(env.GROK_OAUTH_FILE).toBe(path.join(tmpDir, 'cc-heihei', 'grok-oauth.json'))
    expect(env.ANTHROPIC_MODEL).toBe('grok-4.5')
    expect(env.CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST).toBe('1')
    expect(env.CLAUDE_CODE_OAUTH_TOKEN).toBeUndefined()
    expect(env.CC_HEIHEI_OPENAI_OAUTH_PROVIDER).toBeUndefined()
  ...
  test('buildChildEnv passes OpenAI-native effort without leaking Claude effort state', async () => {
    const originalEffort = process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT
    process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT = 'stale-parent-effort'
    try {
      const service = new ConversationService() as any
      const env = (await service.buildChildEnv('/tmp', undefined, {
        providerId: 'openai-official',
      ...
      expect(env.ANTHROPIC_MODEL).toBe('gpt-5.6-sol')
      expect(env.CC_HEIHEI_OPENAI_REASONING_EFFORT).toBe('xhigh')
    } finally {
      if (originalEffort === undefined) delete process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT
      else process.env.CC_HEIHEI_OPENAI_REASONING_EFFORT = originalEffort
    }
  })
  ...
  test('buildChildEnv injects desktop Computer Use host bundle id for sdk sessions', async () => {
    ...
    expect(env.CC_HEIHEI_COMPUTER_USE_HOST_BUNDLE_ID).toBe(
      'com.claude-code-heihei.desktop',
    )
    expect(env.CC_HEIHEI_DESKTOP_SERVER_URL).toBe('http://127.0.0.1:3456')
    expect(env.CC_HEIHEI_TRACE_API_CALLS).toBe('1')
  ...
  test('uses bun entrypoint fallback on Windows dev mode', () => {
    ...
    if (process.platform === 'win32') {
      ...
      expect(args[0]).toContain(path.join('bin', 'claude-heihei'))
    }
  })
  ...
  test('buildChildEnv asks desktop SDK sessions to wait briefly for MCP tools', async () => {
    ...
    expect(env.CC_HEIHEI_DESKTOP_AWAIT_MCP).toBe('1')
    expect(env.CC_HEIHEI_DESKTOP_AWAIT_MCP_TIMEOUT_MS).toBe('5000')
  })
