...
describe('persistent storage upgrade migrations', () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'cc-heihei-persistence-'))
  ...
  test('migrates legacy providers index and writes a backup before changing it', async () => {
    const ccHeiheiDir = path.join(tempDir, 'cc-heihei')
    ...
    expect(report.migratedEntries).toContain('cc-heihei/providers.json')

    const migrated = JSON.parse(await fs.readFile(path.join(ccHeiheiDir, 'providers.json'), 'utf-8')) as {
    ...
    const backups = (await listFiles(ccHeiheiDir)).filter((file) => file.startsWith('providers.json.bak-before-migration-'))
    ...
    const rewritten = JSON.parse(await fs.readFile(path.join(ccHeiheiDir, 'providers.json'), 'utf-8')) as {
      rootFutureField?: unknown
      providers?: Array<Record<string, unknown>>
    }
  ...
  test('imports legacy root providers config into cc-heihei storage without deleting the source', async () => {
    await fs.writeFile(
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('providers.json -> cc-heihei/providers.json')
    expect(report.migratedEntries).toContain('providers.json -> cc-heihei/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(tempDir, 'providers.json'), 'utf-8'))).toMatchObject({
      version: 1,
      activeModel: 'legacy-sonnet',
    })

    const migrated = JSON.parse(await fs.readFile(path.join(tempDir, 'cc-heihei', 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providerOrder?: string[]
      providers?: Array<{
        id?: string
    ...
    const managedSettings = JSON.parse(await fs.readFile(path.join(tempDir, 'cc-heihei', 'settings.json'), 'utf-8')) as {
  ...
  test('does not overwrite current cc-heihei provider storage with a legacy root config', async () => {
    const ccHeiheiDir = path.join(tempDir, 'cc-heihei')
    await fs.mkdir(ccHeiheiDir, { recursive: true })
    await fs.writeFile(
      path.join(tempDir, 'providers.json'),
    ...
    expect(report.failures).toEqual([])
    expect(report.migratedEntries).not.toContain('providers.json -> cc-heihei/providers.json')
    const current = JSON.parse(await fs.readFile(path.join(ccHeiheiDir, 'providers.json'), 'utf-8')) as {
      activeId?: string | null
      providerOrder?: string[]
      providers?: unknown[]
    }
  ...
  test('quarantines malformed managed settings instead of blocking startup', async () => {
    const ccHeiheiDir = path.join(tempDir, 'cc-heihei')
    await fs.mkdir(ccHeiheiDir, { recursive: true })
    await fs.writeFile(path.join(ccHeiheiDir, 'settings.json'), '{"env":', 'utf-8')

    const report = await ensurePersistentStorageUpgraded()

    expect(report.failures).toEqual([])
    expect(report.migratedEntries).toContain('cc-heihei/settings.json')
    expect(JSON.parse(await fs.readFile(path.join(ccHeiheiDir, 'settings.json'), 'utf-8'))).toEqual({})
    const quarantined = (await listFiles(ccHeiheiDir)).filter((file) => file.startsWith('settings.json.invalid-'))
  ...
  test('upgrades existing DeepSeek managed env to follow global thinking settings', async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        env: {
          ...
          CC_HEIHEI_SEND_DISABLED_THINKING: '1',
    ...
    const migrated = JSON.parse(await fs.readFile(path.join(ccHeiheiDir, 'settings.json'), 'utf-8')) as {
      env?: Record<string, string>
    }
    expect(migrated.env?.CC_HEIHEI_SEND_DISABLED_THINKING).toBeUndefined()
    ...
    const backups = (await listFiles(ccHeiheiDir)).filter((file) => file.startsWith('settings.json.bak-before-migration-'))
    expect(backups.length).toBe(1)
  })
})
