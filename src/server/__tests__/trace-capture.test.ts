...
describe('trace capture service', () => {
  ...
  test('keeps a trace-list request on one captured scope across an environment switch', async () => {
    ...
    const traceDirA = path.join(scopeA, 'cc-heihei', 'traces')
  ...
  test('skips malformed trace jsonl entries when reading a session', async () => {
    const traceDir = path.join(tmpDir, 'cc-heihei', 'traces')
  ...
  test('respects managed trace capture settings before writing new records', async () => {
    ...
    const settingsFile = JSON.parse(await fs.readFile(path.join(tmpDir, 'cc-heihei', 'settings.json'), 'utf-8')) as {
  ...
  test('captures direct Anthropic-compatible provider calls from desktop fetch override', async () => {
    ...
    const originalTraceEnv = process.env.CC_HEIHEI_TRACE_API_CALLS
    const originalProviderId = process.env.CC_HEIHEI_TRACE_PROVIDER_ID
    const originalProviderName = process.env.CC_HEIHEI_TRACE_PROVIDER_NAME
    const originalProviderFormat = process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT
    process.env.CC_HEIHEI_TRACE_API_CALLS = '1'
    process.env.CC_HEIHEI_TRACE_PROVIDER_ID = 'provider-sub2api'
    process.env.CC_HEIHEI_TRACE_PROVIDER_NAME = 'Sub2API-ChatGPT'
    process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT = 'anthropic'
    try {
      ...
      if (originalTraceEnv === undefined) delete process.env.CC_HEIHEI_TRACE_API_CALLS
      else process.env.CC_HEIHEI_TRACE_API_CALLS = originalTraceEnv
      if (originalProviderId === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_ID
      else process.env.CC_HEIHEI_TRACE_PROVIDER_ID = originalProviderId
      if (originalProviderName === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_NAME
      else process.env.CC_HEIHEI_TRACE_PROVIDER_NAME = originalProviderName
      if (originalProviderFormat === undefined) delete process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT
      else process.env.CC_HEIHEI_TRACE_PROVIDER_FORMAT = originalProviderFormat
    }
  })
...
describe('session trace API', () => {
  ...
  test('lists trace sessions with storage metadata and managed settings', async () => {
    ...
    expect(body.storageDir).toBe(path.join(tmpDir, 'cc-heihei', 'traces'))
    expect(body.settings).toEqual({
      enabled: true,
      storageDir: path.join(tmpDir, 'cc-heihei', 'traces'),
    })
  })
  ...
  test('deletes a trace session file and invalidates cached reads', async () => {
    ...
    await expect(fs.stat(path.join(tmpDir, 'cc-heihei', 'traces', 'session-delete-trace.jsonl'))).rejects.toThrow()
...
describe('trace read cache', () => {
  ...
  test('falls back to canonical JSONL when the independent trace database is corrupt', async () => {
    ...
    clearTraceCaptureStateForTests()
    const databasePath = path.join(tmpDir, 'cc-heihei', 'db', 'trace-index-v1.sqlite')
  ...
  test('returns canonical trace summaries in shadow mode and records projection mismatches safely', async () => {
    ...
    database.close()
    process.env.CC_HEIHEI_LOCAL_INDEX = 'shadow'
