...
describe('provider presets API', () => {
  ...
  test('configured presets keep current default model ids aligned with official provider docs', () => {
    ...
    expect(kimi?.defaultEnv?.CC_HEIHEI_SEND_DISABLED_THINKING).toBeUndefined()
  ...
  test('configured presets can expose optional API key and promo metadata', () => {
    ...
    expect(zhipu?.promoText).toContain('cc-heihei')
    expect(zhipu?.defaultEnv?.CC_HEIHEI_SEND_DISABLED_THINKING).toBeUndefined()
  ...
  test('GET and PUT /api/providers/settings read and write cc-heihei settings.json', async () => {
    const initial = {
      env: {
    ...
    const updatedRaw = await fs.readFile(path.join(tmpDir, 'cc-heihei', 'settings.json'), 'utf-8')
