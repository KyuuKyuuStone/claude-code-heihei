...
const originalDisableTerminalShellEnv = process.env.CC_HEIHEI_DISABLE_TERMINAL_SHELL_ENV
const originalTaskTimeout = process.env.CC_HEIHEI_TASK_TIMEOUT_MS
const originalLocalAccessToken = process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN
const originalSystemProxyUrl = process.env.CC_HEIHEI_SYSTEM_PROXY_URL
...
function restoreEnv(): void {
  ...
  if (originalDisableTerminalShellEnv) {
    process.env.CC_HEIHEI_DISABLE_TERMINAL_SHELL_ENV = originalDisableTerminalShellEnv
  } else {
    delete process.env.CC_HEIHEI_DISABLE_TERMINAL_SHELL_ENV
  }
  if (originalTaskTimeout) {
    process.env.CC_HEIHEI_TASK_TIMEOUT_MS = originalTaskTimeout
  } else {
    delete process.env.CC_HEIHEI_TASK_TIMEOUT_MS
  }
  if (originalLocalAccessToken !== undefined) {
    process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN = originalLocalAccessToken
  } else {
    delete process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN
  }
  if (originalSystemProxyUrl !== undefined) {
    process.env.CC_HEIHEI_SYSTEM_PROXY_URL = originalSystemProxyUrl
  } else {
    delete process.env.CC_HEIHEI_SYSTEM_PROXY_URL
  }
...
describe('cron scheduler launcher resolution', () => {
  ...
  it('uses a configurable scheduled task timeout with the default unchanged', () => {
    ...
    expect(resolveCronTaskTimeoutMs({ CC_HEIHEI_TASK_TIMEOUT_MS: '1800000' })).toBe(1_800_000)
    expect(resolveCronTaskTimeoutMs({ CC_HEIHEI_TASK_TIMEOUT_MS: 'not-a-number' })).toBe(10 * 60 * 1000)
    expect(resolveCronTaskTimeoutMs({ CC_HEIHEI_TASK_TIMEOUT_MS: '0' })).toBe(10 * 60 * 1000)
  })

  unixOnly('executeTask arms the subprocess timeout from CC_HEIHEI_TASK_TIMEOUT_MS', async () => {
    ...
    process.env.CC_HEIHEI_TASK_TIMEOUT_MS = '12345'
  ...
  it('prefers an explicit CC_HEIHEI_ROOT when it points at a source checkout', async () => {
  ...
  unixOnly('executeTask applies direct, system, and manual network settings to the sidecar', async () => {
    ...
    process.env.CLAUDE_CLI_PATH = sidecarPath
    process.env.CLAUDE_APP_ROOT = appRoot
    process.env.CC_HEIHEI_SYSTEM_PROXY_URL = 'http://127.0.0.1:7897'
