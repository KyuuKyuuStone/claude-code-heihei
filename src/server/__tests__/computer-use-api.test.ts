...
describe('Computer Use API authorized app config', () => {
  ...
  it('persists the Computer Use enabled flag independently', async () => {
    ...
    const raw = await readFile(
      join(configDir!, 'cc-heihei', 'computer-use-config.json'),
      'utf8',
    )
  ...
  it('rejects malformed config patches without writing them', async () => {
    ...
    await expect(
      readFile(join(configDir!, 'cc-heihei', 'computer-use-config.json'), 'utf8'),
    ).rejects.toThrow()
  })

  it('fails closed on a corrupt stored config and refuses to overwrite it', async () => {
    const configPath = join(configDir!, 'cc-heihei', 'computer-use-config.json')
    await mkdir(join(configDir!, 'cc-heihei'), { recursive: true })
