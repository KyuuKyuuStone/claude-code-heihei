...
beforeEach(async () => {
  ...
  await fs.writeFile(
    path.join(configDir, 'cc-heihei', 'providers.json'),
...
describe('DoctorService', () => {
  ...
  test('reports schema-invalid managed providers without exposing parsed contents', async () => {
    ...
    await fs.mkdir(path.join(schemaConfigDir, 'cc-heihei'), { recursive: true })
    await fs.writeFile(
      path.join(schemaConfigDir, 'cc-heihei', 'providers.json'),
      JSON.stringify({ activeId: null, providers: [{ id: 'provider-1' }] }),
      'utf-8',
    )
    ...
    const report = await service.getReport()
    const providers = report.items.find((item) => item.id === 'cc-heihei-providers')
