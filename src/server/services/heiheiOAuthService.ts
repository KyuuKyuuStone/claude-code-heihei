/**
 * HeiheiOAuthService — 桌面端自管 Claude OAuth token
 *
...
export class HeiheiOAuthService {
  ...
  private getOAuthFilePath(): string {
    ...
    return path.join(configDir, 'cc-heihei', 'oauth.json')
  }
  ...
  async ensureFreshTokens(): Promise<StoredOAuthTokens | null> {
    ...
    try {
      ...
      logTokenRefreshFailure('[HeiheiOAuthService]', err)
      return null
    }
  }
...
export const heiheiOAuthService = new HeiheiOAuthService()
