...
export class DoctorService {
  ...
  private async buildTargets(): Promise<DoctorTarget[]> {
    ...
    const targets: DoctorTarget[] = [
      ...
      this.jsonTarget(
        ...
        'Managed providers',
        'user',
        path.join(this.configDir, 'cc-heihei', 'providers.json'),
      ),
      this.jsonTarget(
        'cc-heihei-settings',
      ...
      this.jsonTarget('oauth', 'OAuth tokens', 'user', path.join(this.configDir, 'cc-heihei', 'oauth.json')),
      this.jsonTarget(
        'openai-oauth',
        'OpenAI OAuth tokens',
        'user',
        path.join(this.configDir, 'cc-heihei', 'openai-oauth.json'),
      ),
      this.jsonTarget(
        'grok-oauth',
        'Grok OAuth tokens',
        'user',
        path.join(this.configDir, 'cc-heihei', 'grok-oauth.json'),
      ),
    ]
  ...
  private async inspectJsonTarget(target: DoctorTarget): Promise<DoctorReportItem> {
    ...
    try {
      ...
      if (target.id === 'cc-heihei-providers') {
