 ...
 *
 * Storage: ~/.claude/cc-heihei/providers.json (lightweight index)
 * Active provider env vars written to ~/.claude/cc-heihei/settings.json
...
import { heiheiOpenAIOAuthService } from './heiheiOpenAIOAuthService.js'
import {
  GROK_OFFICIAL_PROVIDER,
  isGrokOfficialProviderId,
} from './grokOfficialProvider.js'
import { heiheiGrokOAuthService } from './heiheiGrokOAuthService.js'
import {
...
export class ProviderService {
  ...
  private getCcHeiheiDir(): string {
    return path.join(this.getConfigDir(), 'cc-heihei')
  }

  private getIndexPath(): string {
    return path.join(this.getCcHeiheiDir(), 'providers.json')
  }
   ...
   *  1. A cc-heihei provider is active → has auth
  ...
  async checkAuthStatus(): Promise<{
    hasAuth: boolean
    source: 'cc-heihei-provider' | 'openai-oauth' | 'grok-oauth' | 'original-settings' | 'env' | 'none'
    activeProvider?: string
  }> {
    // 1. Check cc-heihei active provider
    const index = await this.readIndex()
    if (index.activeId) {
      if (isOpenAIOfficialProviderId(index.activeId)) {
        const tokens = await heiheiOpenAIOAuthService.ensureFreshTokens()
      ...
      if (provider) {
        ...
        if (Object.values(authEnv).some(value => value.length > 0)) {
          return { hasAuth: true, source: 'cc-heihei-provider', activeProvider: provider.name }
        }
      }
    }
