...
async function tempFile(contents: string | Buffer): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'cc-heihei-source-fingerprint-'))
  tempDirs.push(root)
...
describe('source fingerprint transitions', () => {
  ...
  it('round-trips a stable versioned fingerprint through the single prefix_hash field', async () => {
    ...
    expect(stored.startsWith('cc-heihei-source-fingerprint:v2:')).toBe(true)
    ...
    expect(deserializeSourceFingerprint('cc-heihei-source-fingerprint:v2:not-json')).toBeNull()
  })
})
