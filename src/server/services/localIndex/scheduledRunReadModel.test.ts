...
afterEach(async () => {
  ...
  if (originalLocalIndexMode === undefined) delete process.env.CC_HEIHEI_LOCAL_INDEX
  else process.env.CC_HEIHEI_LOCAL_INDEX = originalLocalIndexMode
...
describe('scheduled run read model', () => {
  ...
  test('falls back to the canonical file when the independent database cannot open', async () => {
    ...
    const dbPath = path.join(tmpDir, 'cc-heihei', 'db', 'scheduled-runs-v1.sqlite')
    await fs.mkdir(dbPath, { recursive: true })
    const unrelated = path.join(tmpDir, 'cc-heihei', 'db', 'trace-index-v1.sqlite')
