...
export function prepareManagedDatabasePath(options: {
  ...
  const expectedPath = join(lexicalScope, 'cc-heihei', 'db', options.filename)
  ...
  const ccHeiheiDir = join(lexicalScope, 'cc-heihei')
  const databaseDir = join(ccHeiheiDir, 'db')
  ensureRealManagedDirectory(ccHeiheiDir, trustRoot)
