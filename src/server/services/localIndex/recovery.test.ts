...
async function createTempDir(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'cc-heihei-local-index-recovery-'))
...
describe('local index recovery', () => {
  test('moves only the fixed derived database family into the managed backup directory', async () => {
    ...
    const databasePath = join(scope, 'cc-heihei', 'db', 'index-v1.sqlite')
    ...
    expect(relative(join(scope, 'cc-heihei', 'db', 'backups'), result.backupPath))
      .not.toStartWith('..')
  ...
  test.each(['cc-heihei', 'db', 'backups'] as const)(
  ...
  test('rolls back a partial family move and retains no completed backup', async () => {
    ...
    const backupsRoot = join(scope, 'cc-heihei', 'db', 'backups')
  ...
  test('keeps only the newest three bounded derived backups', async () => {
    ...
    const backups = await readdir(join(scope, 'cc-heihei', 'db', 'backups'))
