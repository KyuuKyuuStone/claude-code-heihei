...
type EnvironmentName = 'HOME' | 'CLAUDE_CONFIG_DIR' | 'CC_HEIHEI_LOCAL_INDEX'
...
async function createTempDir(label: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), `cc-heihei-${label}-`))
...
afterEach(async () => {
  for (const name of ['HOME', 'CLAUDE_CONFIG_DIR', 'CC_HEIHEI_LOCAL_INDEX'] as const) {
...
describe('local index coordinator', () => {
  it('enters a busy cooldown after the first indexed read failure', async () => {
    ...
    const coordinator = createLocalIndexCoordinator({
      ...
      resolveDatabasePath: () => '/tmp/config/cc-heihei/db/index-v1.sqlite',
  ...
  it('stops serving scope A before reopening the runtime index for scope B', async () => {
    ...
    let activePath = '/tmp/config-a/cc-heihei/db/index-v1.sqlite'
    let opens = 0
    let closes = 0
    const indexes = new Map([
      [activePath, createFakeIndex([candidate(1, activeScope)])],
      ['/tmp/config-b/cc-heihei/db/index-v1.sqlite', createFakeIndex([
    ...
    activePath = '/tmp/config-b/cc-heihei/db/index-v1.sqlite'
  ...
  it('converges external append, create, and delete events through the serial writer pump', async () => {
    ...
    const databasePath = join(configDir, 'cc-heihei', 'db', 'index-v1.sqlite')
  ...
  it('keeps a failed exact path degraded across unrelated successful batches until that path converges', async () => {
    ...
    const coordinator = createLocalIndexCoordinator({
      resolveMode: () => ({ mode: 'on', warningCode: null }),
      resolveScope: () => configDir,
      resolveDatabasePath: () => join(configDir, 'cc-heihei', 'db', 'index-v1.sqlite'),
      openDatabase: () => fakeDatabase(() => {}),
      createIndex: () => index,
  ...
  it('auto-recovers a corrupt header once and requires explicit rebuild for a future schema', async () => {
    ...
    expect((await readdir(join(configDir, 'cc-heihei', 'db', 'backups'))).length).toBe(1)
    ...
    expect((await readdir(join(configDir, 'cc-heihei', 'db', 'backups'))).length).toBe(2)
    expect(await Bun.file(source.path).text()).toBe(sourceBefore)
  ...
  it('resolves a fresh stopped-scope path and rebuilds only that derived database family', async () => {
    ...
    const firstPath = join(firstScope, 'cc-heihei', 'db', 'index-v1.sqlite')
    const secondPath = join(secondScope, 'cc-heihei', 'db', 'index-v1.sqlite')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const databaseDir = join(configDir, 'cc-heihei', 'db')
