...
describe('managed SQLite database paths', () => {
  test.each([
    ...
    async (_kind, filename, openDatabase) => {
      ...
      await fs.mkdir(path.join(scope, 'cc-heihei'), { recursive: true })
      await fs.mkdir(outside)
      await fs.symlink(outside, path.join(scope, 'cc-heihei', 'db'))
      const databasePath = path.join(scope, 'cc-heihei', 'db', filename)
  ...
  test('allows the configured trust root itself to be a symlink', async () => {
    ...
    const databasePath = path.join(scope, 'cc-heihei', 'db', 'index-v1.sqlite')
  ...
  test('rejects an existing database-file symlink', async () => {
    ...
    const scope = path.join(root, 'config')
    const databaseDir = path.join(scope, 'cc-heihei', 'db')
