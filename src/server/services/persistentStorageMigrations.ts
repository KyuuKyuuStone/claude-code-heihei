...
async function migrateLegacyRootProviders(
  ...
  const targetPath = path.join(ccHeiheiDir, 'providers.json')
  try {
    await fs.access(targetPath)
    return
  } catch (error) {
    if (errnoCode(error) !== 'ENOENT') {
      report.failures.push(`cc-heihei/providers.json: ${error instanceof Error ? error.message : String(error)}`)
      return
    }
  }
  ...
  try {
    ...
    report.migratedEntries.push('providers.json -> cc-heihei/providers.json')
    ...
    if (managedSettings) {
      ...
      report.migratedEntries.push('providers.json -> cc-heihei/settings.json')
    }
  } catch (error) {
    if (error instanceof SyntaxError) {
