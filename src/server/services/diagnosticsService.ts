...
export class DiagnosticsService {
  ...
  getLogDir(): string {
    return path.join(this.getConfigDir(), 'cc-heihei', 'diagnostics')
  }
  ...
  async recordEvent(input: DiagnosticEventInput): Promise<DiagnosticWriteResult> {
    ...
    // ~/.claude/cc-heihei/diagnostics. Tests that genuinely exercise diagnostics
  ...
  async exportBundle(): Promise<DiagnosticsExportResult> {
    ...
    const fileName = `cc-heihei-diagnostics-${timestamp}.tar.gz`
  ...
  private async buildProvidersSummary(): Promise<Record<string, unknown>> {
    const providerPath = path.join(this.getConfigDir(), 'cc-heihei', 'providers.json')
    try {
