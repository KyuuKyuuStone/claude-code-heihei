...
export function resolveCronTaskTimeoutMs(
  env: { CC_HEIHEI_TASK_TIMEOUT_MS?: string } = process.env,
): number {
  const raw = env.CC_HEIHEI_TASK_TIMEOUT_MS?.trim()
...
export function resolveCronProjectRoot(
  ...
  const explicitRoot = env.CC_HEIHEI_ROOT?.trim()
...
export class CronScheduler {
  ...
  private shouldStripInheritedProviderEnv(providerId?: string | null): boolean {
    ...
    const ccHeiheiDir = path.join(this.getConfigDir(), 'cc-heihei')
    if (existsSync(path.join(ccHeiheiDir, 'providers.json'))) {
      return true
    }

    try {
      const raw = readFileSync(path.join(ccHeiheiDir, 'settings.json'), 'utf-8')
  ...
  private shouldMarkManagedOAuth(providerId?: string | null): boolean {
    ...
    try {
      const raw = readFileSync(
        path.join(this.getConfigDir(), 'cc-heihei', 'settings.json'),
        'utf-8',
      )
      const parsed = JSON.parse(raw) as { env?: Record<string, string> }
      const env = parsed.env ?? {}
