...
</head><body><div class="card"><h1>✓ Grok Login Successful</h1><p>Authorization is complete. You can close this window and return to Claude Code Heihei.</p></div><script>setTimeout(() => window.close(), 3000)</script></body></html>`
...
export class HeiheiGrokOAuthService {
  ...
  getOAuthFilePath(): string {
    return getHeiheiGrokOAuthFilePath()
  }
  ...
  async ensureFreshTokens(): Promise<StoredGrokOAuthTokens | null> {
    ...
    try {
      ...
      logTokenRefreshFailure('[HeiheiGrokOAuthService]', error)
      return null
    }
  }
...
export const heiheiGrokOAuthService = new HeiheiGrokOAuthService()
