 ...
 * (HEIHEI_MARKET_DISABLE_PROVIDERS / HEIHEI_MARKET_BASE_CLAWHUB / HEIHEI_MARKET_BASE_SKILLHUB).
 */
...
export function getProviderBase(source: MarketSource): string {
  const envKey = source === 'clawhub' ? 'HEIHEI_MARKET_BASE_CLAWHUB' : 'HEIHEI_MARKET_BASE_SKILLHUB'
  return process.env[envKey] || DEFAULT_BASES[source]
}

export function isProviderDisabled(source: MarketSource): boolean {
  const disabled = process.env.HEIHEI_MARKET_DISABLE_PROVIDERS
