/**
 * HeiheiOpenAIOAuthService — 桌面端自管 OpenAI OAuth token
 *
 * 为什么存在: macOS Keychain ACL 在 .app 被打上 quarantine 属性后
 * 对无 UI sidecar 静默拒绝,导致 CLI 读不到 OAuth token → 403。
 * 这个 service 把 token 存到 heihei 自己的目录,并通过 env 注入给 CLI。
 *
...
export class HeiheiOpenAIOAuthService {
  ...
  getOAuthFilePath(): string {
    return getHeiheiOpenAIOAuthFilePath()
  }
  ...
  private waitForDesktopCallback(session: OpenAIOAuthSession): void {
    ...
    void listener
      ...
      .catch((err) => {
        ...
        console.error(
          '[HeiheiOpenAIOAuthService] OAuth callback listener failed:',
  ...
  async ensureFreshTokens(): Promise<StoredOpenAIOAuthTokens | null> {
    ...
    try {
      ...
      logTokenRefreshFailure('[HeiheiOpenAIOAuthService]', err)
      return null
    }
  }
...
export const heiheiOpenAIOAuthService = new HeiheiOpenAIOAuthService()
