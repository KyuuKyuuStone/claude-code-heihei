...
// forward as an unknown passthrough key.
...
function normalizeDesktopUiPreferences(value: unknown): DesktopUiPreferences | null {
  ...
  const record = { ...(value as Record<string, unknown>) }
  // schema v5: the desktop pet feature was removed. Strip the legacy key on
  // read so the next write drops it from disk instead of passthrough-carrying
  // it forever.
  delete record.pet
...
export class DesktopUiPreferencesService {
  ...
  private getPreferencesPath(): string {
    return path.join(this.getConfigDir(), 'cc-heihei', 'desktop-ui.json')
  }

  private getProfileDir(): string {
    return path.join(this.getConfigDir(), 'cc-heihei', 'profile')
  }
  ...
  private getProfileAvatarPath(avatarFile: string): string {
    ...
    return path.join(this.getConfigDir(), 'cc-heihei', normalized)
  }
  ...
  async readPreferences(): Promise<DesktopUiPreferencesReadResult> {
    ...
    const preferences = await readRecoverableJsonFile({
      filePath,
      label: 'cc-heihei desktop UI preferences',
