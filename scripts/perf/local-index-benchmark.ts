...
export async function runDeterministicAcceptance(
  ...
  const rootDir = await mkdtemp(join(tmpdir(), 'cc-heihei-local-index-acceptance-'))
  const homeDir = join(rootDir, 'home')
  const configDir = join(homeDir, '.claude')
  const databasePath = join(configDir, 'cc-heihei', 'db', 'index-v1.sqlite')
  const originalHome = process.env.HOME
  const originalConfigDir = process.env.CLAUDE_CONFIG_DIR
  const originalLocalIndexMode = process.env.CC_HEIHEI_LOCAL_INDEX
  let coordinator: LocalIndexCoordinator | undefined
  ...
  try {
    ...
    restoreEnvironment('CC_HEIHEI_LOCAL_INDEX', originalLocalIndexMode)
...
export async function runBenchmark(
  ...
  const rootDir = await mkdtemp(join(tmpdir(), 'cc-heihei-local-index-benchmark-'))
  const homeDir = join(rootDir, 'home')
  const configDir = join(homeDir, '.claude')
  const databasePath = join(configDir, 'cc-heihei', 'db', 'index-v1.sqlite')
  const originalHome = process.env.HOME
  ...
  try {
    ...
    process.env.HOME = homeDir
    process.env.CLAUDE_CONFIG_DIR = configDir
    process.env.CC_HEIHEI_LOCAL_INDEX = options.mode === 'file'
      ? 'off'
      : options.mode === 'shadow'
        ? 'shadow'
        : 'on'
    const localAccessToken = createHash('sha256')
      ...
      .digest('base64url')
    process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN = localAccessToken
    ...
    restoreEnvironment('CLAUDE_CONFIG_DIR', originalConfigDir)
    restoreEnvironment('CC_HEIHEI_LOCAL_INDEX', originalLocalIndexMode)
    restoreEnvironment('CC_HEIHEI_LOCAL_ACCESS_TOKEN', originalLocalAccessToken)
    if (cleanupError) throw cleanupError
  }
