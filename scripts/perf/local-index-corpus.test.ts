...
async function tempRoot(label: string): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), `cc-heihei-${label}-`))
  tempDirs.push(root)
  return root
}
...
function restoreEnvironment(
  name: 'HOME' | 'CLAUDE_CONFIG_DIR' | 'CC_HEIHEI_LOCAL_ACCESS_TOKEN',
...
describe('local index file benchmark', () => {
  ...
  it('enforces the real loopback product deadline and still cleans isolated state', async () => {
    ...
    process.env.CC_HEIHEI_LOCAL_ACCESS_TOKEN = 'benchmark-parent-local-access-token'
    const options = benchmark.parseBenchmarkArgs!([
      '--sessions',
      '50',
      '--entries-per-session',
